...
> 최종 결론과 근거는 [`docs/최종_분석_보고서.md`](./docs/최종_분석_보고서.md)에, 한 장 요약은 [`docs/포트폴리오_요약.md`](./docs/포트폴리오_요약.md)에 있다.

## 예측 목표

> 개강 후 25일 시점에 아직 재학 중인 학생을 대상으로, 최초 25일까지 관측 가능한 정보만 사용해 25일 이후 중도 이탈(`Withdrawn`)을 예측한다.

- 분석 단위는 학생-과목-학기 수강 1건이다(모델 대상 27,654건, 학생 24,947명). 노트북·보고서의 "N명"은 수강 건 수다.
- 25일 전에 이미 이탈한 학생(전체 이탈의 47%)은 주 모델에서 제외한다. 이들은 별도의 초기 이탈 과제로 다룬다.
- 초안은 개강 4주차(28일) 기준이었다. "28일 이내 이탈" 타깃의 시간 누수를 발견해 문제를 "그 시점 재학생의 이후 이탈"로 다시 정의했고, 같은 조건으로 25·28·32일을 비교해 25일로 조정했다. 25일이 개입 대상을 가장 많이 남기고 성능도 가장 높았다(보고서 2.2–2.3절).
...
| 관측창 결정 | `02_Window_Integration/01_Window_Definition` | 21–35일 신호 분석, 랜드마크 코호트 기준 25·28·32일 비교(5절), 25일 확정 |
...
기획, 데이터 이해, 개별·조인 EDA, 가설 검증, 관측창 후보 분석까지는 스터디원들과 함께 진행했다. 이후 관측창 확정, 통합 테이블, 정제, 모델링, 조기 경고 체계, 개입 가이드, 과목 설계 제안은 개인 프로젝트로 이어갔다.
